...
class FCEUXGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Cats's FCEUX 0.1.1B")
    ...
    def setup_display(self):
        ...
        self.status = Label(
            self.root,
            text="Cats's FCEUX 0.1.1B | Ready", 
            bg="#1e1e1e",
            fg="#00ff00",
            font=("Courier", 9),
            bd=1,
    ...
    def show_controls(self):
        ...
        msg = """Cats's FCEUX 0.1.1B Controls:
...
© 2025 AC Software"""
    ...
    def update_image(self):
        ...
        # NOTE: Depending on Tk version, you may need to adapt this
        # to use PhotoImage(data=...) instead of put(data=...).
        self.img.put(data=header + pixels)

    def update_status(self, text):
        """Update status bar"""
        fps_text = f" | {self.fps} FPS" if self.running else ""
        self.status.config(text=f"Cats's FCEUX 0.1.1B | {text}{fps_text}")
    ...
    def run(self):
        """Start GUI main loop"""
        print("═"*60)
        print(" CATS'S FCEUX 0.1.1B - ADVANCED NES EMULATOR 2025")
        print("═"*60)
